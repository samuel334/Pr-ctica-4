...
void selection(float *max, float *min, float *avg, int p_lgth, int c_lgth, int parent[p_lgth][c_lgth], const int population[p_lgth][c_lgth], float (*f_fitness)(int lgth, const int chromosome[lgth]), float p){
	int i, j, k, l, temp, r;
	float flip;
	float *fitness, f_sum = 0; //aptitud y suma de aptitudes
	int less_apt, more_apt, *index = malloc(sizeof(int)*p_lgth);
	/*obtener aptitud e inicializar arreglo de indices*/
	fitness = malloc(sizeof(float)*p_lgth);
	for(i = 0;i<p_lgth;i++){
		index[i] = i;
		fitness[i] = f_fitness(c_lgth, population[i]);
		f_sum += fitness[i];
	}
	/*obtener minimo, maximo y promedio de generacion actual*/
	*max = *avg = 0.0;
	*min = fitness[i];
	for(i = 0;i<p_lgth;i++){
	...
	*min /= f_sum;
	*max /= f_sum;
	*avg /= ((float)p_lgth*f_sum);
	/*seleccion por torneo*/
	i = 0;
	while(i<p_lgth){
		/*barajar*/
		for(j = 0;j<(p_lgth - 1);j++){
			r = rand()%(p_lgth - j);
			temp = index[p_lgth - 1 - j];
			index[p_lgth - 1 - j] = index[r];
			index[r] = temp;
		}
		/*seleccionar padres*/
		j = 0;
		while(j<p_lgth){
			...
			flip = (float)((double)rand()/(double)RAND_MAX);
			k = ( (flip<=p) ? more_apt : less_apt );	//se decide si elegir al mas apto o al menos apto
			/*se toma al padre elegido*/
			for(l = 0;l<c_lgth;l++){
				parent[i][l] = population[k][l];
			}
			i++;
		}
	}
...
int main(int argc, char *argv[]){
	...
	int p_lgth = 16; //numero de individuos por generacion
	...
	for(i = 0;i<gen;i++){
		/*seleccion*/
		selection(&max[i], &min[i], &avg[i], p_lgth, c_lgth, parent, chromosome, getFitness, 0.7);
		/*cruza*/
		breeding(p_lgth, c_lgth, child, parent);
		/*mutar*/
		mutation(p_lgth, c_lgth, child, 0.3, 0.3);
